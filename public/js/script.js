...
function restoreFormData() {
  ...
  if (savedData) {
    ...
    document.querySelector(
      `input[name="low-power"][value="${formData.lowPower || "disabled"}"]`
    ).checked = true;
    document.querySelector(
...
function getFormJsonString() {
  let formData = {
    ...
    LOW_POWER: (
      document.querySelector('input[name="low-power"]:checked').value ==
      "enabled"
    ).toString(),
    CAYENNE_LPP_: (
...
function validateForm() {
  ...
  const inputs = [
    ...
    {
      element: document.getElementById("frame-delay"),
      min: 8,
      max: 100,
      name: "Frame Delay",
    },
